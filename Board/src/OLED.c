...
************************************************************************/
  ...
  #define OLED_SCL  PTD5_OUT
  #define OLED_SDA  PTD4_OUT
  #define OLED_RST  PTD3_OUT
  #define OLED_DC   PTD2_OUT
  #define OLED_CS   PTD1_OUT
...
0x00,0x00,0x03,0x02,0x02,0x02,0xFF,0x02,0x02,0x02,0x02,0x03,0x00,0x00,/*"中",0*/

0x00,0x60,0x58,0xC6,0x60,0x30,0x20,0x20,0xFE,0xA2,0xA4,0x98,0x80,0x00,
0x00,0x24,0x27,0x14,0x12,0x43,0x21,0x21,0x1F,0x18,0x24,0x42,0x70,0x00,/*"线",1*/

0x00,0x80,0x60,0xBC,0x02,0x50,0xA8,0xA8,0x2A,0xAC,0x28,0xF8,0x88,0x00,
0x00,0x00,0x00,0x7F,0x30,0x0F,0x7F,0x04,0x3F,0x04,0x3F,0x44,0x7F,0x00,/*"偏",2*/

0x00,0x00,0x00,0x50,0x56,0xC8,0x70,0x50,0x48,0x56,0x50,0x08,0x00,0x00,
0x00,0x21,0x11,0x49,0x45,0x4B,0x49,0x49,0x79,0x49,0x45,0x41,0x41,0x00,/*"差",3*/

0x00,0x00,0x18,0x68,0x48,0x68,0xDA,0x44,0x28,0xC8,0x28,0x04,0x00,0x00,
0x00,0x02,0x0A,0x09,0x0A,0x0B,0x7E,0x0A,0x09,0x08,0x09,0x0B,0x08,0x00,/*"率",4*/

0x00,0x00,0x10,0x10,0xD0,0x3E,0x10,0x10,0x10,0x10,0x10,0x08,0x00,0x00,
0x00,0x20,0x18,0x46,0x41,0x41,0x41,0x7F,0x41,0x41,0x41,0x60,0x40,0x00,/*"左",5*/

0x00,0x10,0x10,0x10,0x90,0x70,0x0E,0x10,0x10,0x10,0x10,0x10,0x00,0x00,
0x00,0x10,0x0C,0x02,0x3F,0x21,0x21,0x21,0x21,0x21,0x7F,0x01,0x00,0x00,/*"右",6*/
...
void OLED_Draw_Point(unsigned char x,unsigned char y)
{
    ...
    unsigned char Draw_Temp[8][128] = {0};
    unsigned char num_show,dot_y;
	num_show=y%8;
	dot_y=y/8;
	Draw_Temp[dot_y][x]=(Draw_Temp[dot_y][x]|(1<<(num_show)));	
	OLED_Set_Pos(x,dot_y);
    
	if(x%2)//为高位，先写低位
    {
        OLED_Write_dc(Draw_Temp[dot_y][x-1],1);
    }
	OLED_Write_dc(Draw_Temp[dot_y][x],1);
}
...
void displayImage(uint8 dst[HANG][LIE])
{
  ...
  for(j=0;j<8;j++)
  {
    ...
    {
      dataTemp   = 0;
      dataTemp  += 0x01&dst[j * 8 + 0][i];
      dataTemp  += 0x02&dst[j * 8 + 1][i];
      dataTemp  += 0x04&dst[j * 8 + 2][i];
      dataTemp  += 0x08&dst[j * 8 + 3][i];
      dataTemp  += 0x10&dst[j * 8 + 4][i];
      dataTemp  += 0x20&dst[j * 8 + 5][i];
      dataTemp  += 0x40&dst[j * 8 + 6][i];
      dataTemp  += 0x80&dst[j * 8 + 7][i];
      OLED_Write_dc(dataTemp, 1);
...
/***********
***函数名称；OLED_Draw_Line();
***参    数； (x1,y1)：起始坐标 x0:0~127 y0:0~63
***          (x2,y2)：终止坐标 x1:0~127 y1:0~63
***返回值； 无
***函数功能；画任意直线
***********/
void OLED_Draw_Line(unsigned char x1,unsigned char y1,unsigned char x2,unsigned char y2)
{
   ...
   dx = dx * dx_sym; //求绝对值
   dy = dy * dy_sym;
   if(dx >= dy) //斜率小于1时以x轴为基准
   {
     distance = dy-dx;//Bresenham算法中是dy*2-dx ,这里改为1倍
     while(x1 != x2)
    {
      if(distance < 0)
     {
     ...
     {
       distance += (dy-dx);  //dy*2-dx
       y1 += dy_sym;
     }
     OLED_Draw_Point(x1,y1);
     x1 += dx_sym;
    }
    OLED_Draw_Point(x2,y2); //显示最后一点
   }
   else         //斜率大于1以y轴为基准
   {
     distance = dx-dy;
